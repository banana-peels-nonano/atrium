...
| `test_spec_loader_missing_section_refused` (parametrized over the 6 sections) + `test_spec_loader_missing_literal_refused` (parametrized over the 2 literals) + `test_spec_loader_empty_stub_refused` | each missing section / missing "no authority"/"stateless" literal → `SpecInvalid` naming it; an empty stub file fails loudly (originally the repo's Phase-0 stubs, since filled by A9 — pinned via an empty fixture) | fixture specs | spec format fail-closed |
