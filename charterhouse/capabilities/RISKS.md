...
| R3 | Family derivation misclassifies a model id (e.g. a provider-prefixed id), silently weakening the cross-family guarantee | medium | medium | ambiguity | derivation isolated in one function, tested over the committed catalog's ids; tier recorded honestly so a misclass shows in the audit trail; A2 cross-note: additive `Model.family` field retires the heuristic | code (`critic.py::_family`) + test + doc |
