...
| R9 | ~~Ranking weights drift from docs/33 intent because they live in code defaults, not config~~ **RETIRED 2026-07-19 (feat/a2-accessors):** the additive `Config.memory` accessor landed — routes.yaml carries the committed `memory:` block (strict-key validated, docs/33 defaults) and `RetrievalWeights.from_config(config.memory)` is the one wiring line (field-for-field mirror, round-trip tested) | — | — | ambiguity | additive `Config.memory` + `MemoryConfig` + `from_config` landed with A2's accessors branch | code + test (`test_retrieval_weights_from_config`) |
