...
| R9 | `chain._catalog_ids` reads Config's internal model table (no frozen listing seam exists) | Med | Low | architectural-integrity | isolated in one function with a loud note; **cross-note for A2**: additive `Config.models()` seam (docs/43 §7) replaces it | IMPLEMENTATION §6.7 + code comment |
